<x-layout title="Cybersecurity Blog">
  ...
  <section class="bg-gradient-to-r from-dark-blue to-soft-blue text-white relative overflow-hidden">
    ...
    <div class="container mx-auto px-4 py-16 max-lg:py-12 max-sm:py-10 relative z-10">
      <div class="max-w-4xl">
        ...
        <h1 class="text-4xl font-bold mb-3 max-md:text-3xl max-sm:text-2xl">Cybersecurity Blog</h1>
        <p class="text-blue-100 max-w-2xl text-lg leading-relaxed max-sm:text-base mb-6">Explore our collection of insights, research, and practical advice from cybersecurity experts on the latest threats and best practices</p>
        
        <!-- Enhanced search bar -->
        <form action="{{ route('blogs') }}" method="GET" class="relative max-w-md mt-4">
          <input 
            type="text" 
            name="search" 
  ...
  <!-- Main content -->
  <main id="blog" class="bg-gray-50 min-h-screen py-12 max-sm:py-8">
    <div class="container mx-auto px-4">
      {{-- Show search results banner when searching with results --}}
      @if($searchTerm && $posts->count() > 0)
        <div class="mb-8 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-100 px-6 py-5 relative overflow-hidden">
          ...
          <div class="flex items-center justify-between relative z-10">
            <div>
              ...
              <p class="text-gray-600 mt-1">Found {{ $posts->total() }} article{{ $posts->total() !== 1 ? 's' : '' }} matching your search</p>
            </div>
            <a href="{{ route('blogs') }}" class="flex items-center text-sm font-medium text-blue-700 hover:text-blue-800 bg-white px-4 py-2 rounded-lg shadow-sm border border-blue-100 hover:shadow-md transition-all duration-200">
      ...
      <!-- Featured article section with enhanced design -->
      {{-- Only show featured articles if not searching --}}
      @unless($searchTerm)
        <div class="mb-12">
          @if($posts->count() > 0)
            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 max-sm:p-4">
              <div class="flex items-center mb-6">
                ...
                <h2 class="text-xl font-bold text-gray-800">Featured & Popular Articles</h2>
              </div>
              
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                @foreach($posts->sortByDesc('views')->take(3) as $featuredPost)
                  <a href="{{ route('blogs.show', $featuredPost->id) }}" class="group">
                    <div class="relative rounded-xl overflow-hidden aspect-video mb-3 border border-gray-100 shadow-sm">
                      <img src="{{ Storage::url($featuredPost->image_url) ?? '/img/blog-placeholder.jpg' }}" class="w-full h-full object-cover transform group-hover:scale-105 transition duration-300" alt="{{ $featuredPost->title }}">
                      ...
                      <div class="absolute bottom-0 left-0 right-0 p-4">
                        ...
                        <h3 class="text-white font-medium line-clamp-2 text-lg group-hover:underline decoration-2 underline-offset-2">{{ $featuredPost->title }}</h3>
                      </div>
                    </div>
                  </a>
                @endforeach
      ...
      <!-- Topic badges for visual categorization (non-functional) -->
      @unless($searchTerm)
      ...
      <!-- Blog post grid with enhanced design -->
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 max-sm:gap-4">
        @forelse ($posts as $post)
          <a href="{{ route('blogs.show', $post->id) }}" class="hover:no-underline blog-card group">
            <div class="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition duration-300 h-full flex flex-col border border-gray-100">
              ...
              <div class="relative aspect-video">
                <img src="{{ Storage::url($post->image_url) ?? '/img/blog-placeholder.jpg' }}" class="w-full h-full object-cover" alt="{{ $post->title }}">
              ...
              <div class="p-5 flex-grow flex flex-col">
                <h2 class="font-bold text-gray-800 mb-3 line-clamp-2 text-lg group-hover:text-soft-blue transition-colors duration-200">{{ $post->title ?? 'Untitled Post' }}</h2>
                
                <!-- Preview of content -->
                <p class="text-gray-600 text-sm line-clamp-2 mb-3">{{ \Illuminate\Support\Str::limit(strip_tags($post->content), 100) }}</p>
                ...
                <div class="mt-auto pt-3 flex items-center justify-between text-sm text-gray-500 border-t border-gray-100">
                  <div class="flex items-center space-x-4">
                    ...
                    <div class="flex items-center">
                      ...
                      {{ $post->created_at->format('M d, Y') ?? 'Date unknown' }}
                    </div>
                  </div>
                </div>
          ...
          <div class="col-span-full relative overflow-hidden py-16 sm:py-24 text-center bg-gradient-to-b from-white to-gray-50 rounded-2xl border border-gray-100 shadow-sm">
            ...
            <div class="absolute top-0 left-0 -translate-x-1/4 -translate-y-1/4 opacity-10">
              <svg width="404" height="404" fill="none" viewBox="0 0 404 404" role="img" aria-labelledby="svg-squares">
                <title id="svg-squares">Background Squares</title>
                <defs>
            ...
            <div class="relative z-10">
              <div class="bg-gradient-to-br from-blue-100 to-indigo-200 h-20 w-20 rounded-full flex items-center justify-center mx-auto mb-6 shadow-lg border-4 border-white">
                {{-- Updated Icon for Search Results --}}
                @if($searchTerm)
                ...
                @else
                  {{-- Original Icon for No Posts --}}
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                @endif
              </div>
              {{-- Different message based on whether it's a search result or just no posts --}}
              @if($searchTerm)
                ...
                <p class="text-gray-600 max-w-md mx-auto mb-8 text-base leading-relaxed">We couldn't find any articles matching your search. Try refining your keywords or browse all available posts.</p>
                <a href="{{ route('blogs') }}" class="inline-flex items-center justify-center px-7 py-3 bg-soft-blue hover:bg-blue-700 text-white font-medium rounded-xl transition-all duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5">
              ...
              @else
                {{-- Original No Posts Message --}}
                <h3 class="text-xl sm:text-2xl font-semibold text-gray-900 mb-3">No Blog Posts Yet</h3>
                <p class="text-gray-600 max-w-md mx-auto mb-8 text-base leading-relaxed">We're busy crafting insightful cybersecurity content. Check back soon for new articles and updates!</p>
                ...
                <div class="max-w-md mx-auto mt-10 px-6 py-6 bg-white/50 backdrop-blur-sm rounded-xl border border-gray-200/70 shadow-sm">
                  <h4 class="font-medium text-gray-800 mb-3 text-base">Get notified when new articles are published</h4>
      ...
      @if($posts->hasPages())
        <div class="mt-12 flex justify-center">
          {{ $posts->appends(['search' => $searchTerm])->links() }}
        </div>
      @endif
    </div>
  </main>
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      filterButtons.forEach(button => {
        button.addEventListener('click', () => {
          ...
          filterButtons.forEach(btn => btn.classList.remove('active', 'bg-[#3498DB]', 'text-white'));
          filterButtons.forEach(btn => btn.classList.add('bg-gray-100', 'text-gray-700'));
          button.classList.remove('bg-gray-100', 'text-gray-700');
          button.classList.add('active', 'bg-[#3498DB]', 'text-white');
